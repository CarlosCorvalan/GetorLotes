...
(function () {
  ...
  function injectStyles() {
    var css = ''
      + '#ds-fab{position:fixed;z-index:5000;bottom:110px;right:14px;background:#0d4a8f;color:#fff;'
  ...
  function renderTabDetectar() {
    ...
    body.innerHTML =
      ...
      '<button id="ds-btn-detectar" class="ds-btn" style="width:100%;background:#0d4a8f;margin-top:12px;padding:11px">&#128752; Buscar candidatos</button>' +
  ...
  async function ejecutarDeteccion() {
    ...
    btn.disabled = true;
    btn.innerHTML = 'Consultando Sentinel Hub... (puede tardar 20-60s)';
    res.innerHTML = '';
    ...
    btn.disabled = false;
    btn.innerHTML = '&#128752; Buscar candidatos';
  }
  ...
  function pintarPreview(candidatos, lote) {
    ...
    candidatos.forEach(function (c, i) {
      L.geoJSON({ type: 'Feature', geometry: c.geojson }, { style: { color: '#c0392b', weight: 2, fillColor: '#c0392b', fillOpacity: .35 } })
  ...
  async function renderTabLista() {
    ...
    body.innerHTML = _cache.map(function (c) {
      ...
      if (c.estado === 'pendiente') {
        ...
        acciones = '<div style="font-size:.7rem;color:#f59e0b;margin-top:5px">Esperando confirmación del inspector desde el celular</div>';
      }
  ...
  function pintarCandidatosGuardados(lista) {
    ...
    lista.forEach(function (c) {
      if (!c.geojson) return;
      var col = c.estado === 'pendiente' ? '#3b82f6' : c.estado === 'asignado' ? '#f59e0b' : '#10b981';
      L.geoJSON({ type: 'Feature', geometry: c.geojson }, { style: { color: col, weight: 2, fillColor: col, fillOpacity: .35 } })
        .bindTooltip((c.estado || '').toUpperCase() + ' · NDVI ' + c.ndvi_promedio)
        .addTo(lgCandidatos);
    });
  }

  // ---------- Asignación (prompts simples; no depende de modales del HTML) ----------
  function abrirAsignacion(id) {
    var nombre = prompt('Nombre del inspector a asignar:');
    if (!nombre || !nombre.trim()) return;
    var num = prompt('WhatsApp del inspector (con código de país, sin +), o dejá vacío para notificar solo por app:');
    asignar(id, nombre.trim(), (num || '').trim());
  }

  async function asignar(id, nombre, wsp) {
    var c = _cache.find(function (x) { return x.id === id; });

    // Abrimos WhatsApp ANTES de esperar la respuesta de Supabase: si lo
    // hacemos después del await, el navegador ya no lo reconoce como una
    // acción directa del click del usuario y lo bloquea (o lo abre como
    // ventana emergente en vez de mandarte directo a la app/web de WhatsApp).
    if (wsp && c) {
      var lote = (window.LP || []).find(function (l) { return l.id === c.lote_id; });
      var nombreLote = lote ? ((lote.proyecto ? '[' + lote.proyecto + '] ' : '') + lote.campo) : c.lote_id;
      var msg = encodeURIComponent(
        '🛰️ Candidato a verificar (detección satelital)\n\n' +
        'Inspector: ' + nombre + '\n' +
        'Lote: ' + nombreLote + '\n' +
        'NDVI detectado: ' + c.ndvi_promedio + ' — Área aprox: ' + c.area_ha + ' ha\n\n' +
        'Abrí la app para ver la ubicación y confirmar en el lote correspondiente.'
      );
  ...
  async function descartar(id) {
    ...
    await supa.from('candidatos_deteccion').update({ estado: 'descartado', resuelto_en: new Date().toISOString() }).eq('id', id);
    renderTabLista();
  }

  // ---------- Confirmación en campo (solo mobile: reusa el flujo existente) ----------
  function iniciarConfirmacionEnCampo(id) {
    var c = _cache.find(function (x) { return x.id === id; });
    if (!c) return;
